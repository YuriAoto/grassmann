...
\begin{tikzpicture}[
  ...
  % ======
  \node (init)  at (\X{10}, \Y{-15})
  [algstep, text width=\W{60}]
  {
    \textbf{Initialisation}\vspT
    {$\Psi_{CC}$ as instance of \texttt{wave\_functions.IntermNormWF}}\vsp
  };
  % ------
  ...
  {
    \textbf{Energy}\vspT
    \texttt{wave\_functions.IntermNormWF.energy}\vsp
  };
  % ------
  ...
  [algstep, text width=\W{50}, hyperlink node=sec:ccsd_update]
  {
    ...
    \texttt{coupled\_cluster.ccsd.step}\vsp
  };
  % ------
  ...
  \draw [->] (cc_version.270) .. controls +(down:1) and +(up:1) .. (res.115)
