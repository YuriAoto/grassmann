...
\begin{tikzpicture}[
  ...
  % ======
  \node (init)  at (\X{15}, \Y{-15})
  [algstep, text width=\W{30}, hyperlink node=sec:]
  {
    \textbf{Input}\vspT
    {The $\Psi_0$ wave function}\vsp
  ...
  [algstep, text width=\W{50}, hyperlink node=sec:]
  {
    \textbf{$\Psi_{CC}$ as a FCI wave function}\vspT
    \texttt{fci.WaveFunctionFCI.from\_CC}\vsp
  };
  % ------
  ...
  [algmultistep, text width=\W{50}, hyperlink node=sec:dist_cc_man]
  {
    ...
    \begin{minipage}[t][5cm]{1.0\textwidth}
      \centering
      \texttt{coupled\_cluster.manifold.make\_min\_dist\_jac\_hess}
    \end{minipage}
  };
  ...
  % ======
  \node (update) at (\X{25}, \Y{105})
  [algstep, text width=\W{50}]
  {
    \textbf{Update amplitudes}\vspT
    \texttt{fci.WaveFunctionFCI.calc\_dist\_to\_CC\_manifold}\vsp
