...
class Molpro_FCI_Wave_Function():
    ...
    def check_Newton_Absil_eq(self, U, eta, eps = 0.001):
        ...
        def calc_grad(this_U):
            ...
            for i in range(grad_a.shape[0]):
                for j in range(grad_a.shape[1]):
                    ...
                    grad_a[i,j] = self.distance_to_det(Ua_Ub)
                    Ua = np.copy(this_Ua)
                    Ua[i,j] = Ua[i,j] - eps
                    Ua_Ub = (orth(Ua), orth(this_Ub)) if do_orth else (Ua, this_Ub)
                    grad_a[i,j] = (grad_a[i,j] - self.distance_to_det(Ua_Ub))/(2*eps)
            if not restricted:
                ...
                for i in range(grad_b.shape[0]):
                    for j in range(grad_b.shape[1]):
                        ...
                        grad_b[i,j] = self.distance_to_det(Ua_Ub)
                        Ub = np.copy(this_Ub)
                        Ub[i,j] = Ub[i,j] - eps
                        Ua_Ub = (orth(this_Ua), orth(Ub)) if do_orth else (this_Ua, Ub)
                        grad_b[i,j] = (grad_b[i,j] - self.distance_to_det(Ua_Ub))/(2*eps)
            if restricted:
                return grad_a
            else:
