...
def build_analytic_hessian(C_a, C_b, S, h, g):
    ...
    if N_a:
        ...
        for j in range(N_a):
            ...
            e_j[j] = 0.0
        hess[:n*N_a, :n*N_a] -= np.kron(energies_a.T, S)
...
class GradientTestFock(unittest.TestCase):
    ...
    def test_H2_ccpvdz(self):
        ...
        analytic_hess = build_analytic_hessian(C, C, S, h, g)
        numeric_hess = build_numeric_hessian(C, C, S, h, g)

        self.assertEqual(numeric_hess, numeric_hess.T)
